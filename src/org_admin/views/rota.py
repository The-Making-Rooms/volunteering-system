...
def get_available_volunteers_discrete(
    opportunity_id: int,
    omit_confirmed: bool = False,
    selected_role: Optional[int] = None,
    selected_date: Optional[int] = None,
) -> List[Dict[str, Any]]:
    """
    Build a list of shift blocks for per-role scheduling: each role has its own OneOffDate(s).

    Performance improvements:
    - Eliminates per-role Section queries via prefetch_related.
    - Batches schedule resolution: one probe for the selected slot, then a single filtered query.
    - Uses select_related on organisation where schedules are loaded.
    - Computes volunteer lists in a tight loop after assembling minimal role/schedule pairs.

    Behavior preserved:
    - Filtering semantics for selected_role and selected_date match the original.
    - Keys and value shapes in returned 'shifts' are unchanged.
    - omit_confirmed maps to omit_unconfirmed for non-section assignments as before.
    """
    # Roles: either one role by ID or all roles on the opportunity, with Sections prefetched
    if selected_role:
        roles_qs = Role.objects.filter(id=selected_role)
    else:
        roles_qs = Role.objects.filter(opportunity=opportunity_id).select_related("opportunity__organisation")

    roles_qs = roles_qs.prefetch_related(
        Prefetch("section_set", queryset=Section.objects.all(), to_attr="prefetched_sections")
    )
    roles = list(roles_qs)

    # Resolve schedules for roles:
    # If selected_date is provided, first resolve its (date, start_time, end_time) to reuse as a slot filter.
    selected_slot = None
    if selected_date:
        try:
            selected_slot = OneOffDate.objects.only("date", "start_time", "end_time", "opportunity").get(id=selected_date)
        except ObjectDoesNotExist:
            # No schedules match; return early for speed
            return []

    # Collect role IDs to query schedules in batches (reduces per-role queries)
    role_ids = [r.id for r in roles]
    base_schedules = OneOffDate.objects.select_related("opportunity__organisation")

    if selected_slot:
        schedules_qs = base_schedules.filter(
    ...
    # Materialize schedules and bucket them by role_id for O(1) lookup during cross-product
    schedules_by_role: Dict[int, List[OneOffDate]] = {}
    for s in schedules_qs:
        schedules_by_role.setdefault(s.role_id, []).append(s)

    # Build shifts with prefetched sections and attached volunteer info
    shifts: List[Dict[str, Any]] = []
    for role in roles:
        role_sections = getattr(role, "prefetched_sections", [])
        role_schedules = schedules_by_role.get(role.id, [])

        for schedule in role_schedules:
            if role_sections:
                # Sectioned roles: compute per-section assigned/accepted
                for section in role_sections:
                    shifts.append({
                        "role": role,
                        "schedule": schedule,
                        "available_volunteers": get_shift_volunteers(schedule.id, role.id),
                        "assigned_volunteers": get_assigned_volunteers(
                            schedule.id, role.id, section.id, omit_unconfirmed=omit_confirmed
                        ),
                        "accepted_volunteers": get_accepted_and_confirmed(
                            schedule.id, role.id, section.id
                        ),
                        "section": section,
                    })
            else:
                ...
                shifts.append({
                    ...
                    "available_volunteers": get_shift_volunteers(schedule.id, role.id),
                    "assigned_volunteers": get_assigned_volunteers(
                        schedule.id, role.id, omit_unconfirmed=omit_confirmed
                    ),
...
def get_available_volunteers_discrete_old(opportunity_id: int, omit_confirmed: bool=False, selected_role=None, selected_date=None):
    """
    Build a list of shift blocks for per-role scheduling: each role has its own OneOffDate(s).
...
# Assumes these helpers and models exist in the project, as in the original:
# - require_authenticated, assert_org_access, safe_check_if_hx
# - get_shift_volunteers, get_assigned_volunteers, get_accepted_and_confirmed
# - get_available_volunteers_discrete
# - Opportunity, Role, Section, OneOffDate, VolunteerShift

def assign_rota(request: HttpRequest, opp_id: int, success: Optional[str] = None, error: Optional[str] = None) -> HttpResponse:
    """
    Fast, drop‑in replacement of the shift assignment interface.

    Key performance improvements:
    - Reduces N+1 queries by prefetching Roles -> Sections and Opportunity -> Organisation.
    - Batches schedule (OneOffDate) retrieval with targeted filtering and value-based slot aggregation.
    - Avoids per‑iteration Role and Section queries by materializing maps in memory.
    - Defers expensive per‑shift volunteer calculations until the final pass, operating on an already compact list.
    - Filters unconfirmed shifts with a single DB pass, then post-filters in Python only when necessary.

    Behavior:
    - Maintains original filtering semantics for role/date from GET/POST.
    - Preserves SHARED vs PERROLE (discrete) rota behavior.
    - Keeps context keys and template compatibility intact.
    """
    ...
    # Load all roles for the sidebar/filter and to build maps
    all_roles_qs = Role.objects.filter(opportunity=opportunity).select_related("opportunity__organisation")
    roles = list(all_roles_qs)

    # Parse filters from GET/POST, preserving original semantics
    get_role = request.GET.get("role")
    get_date = request.GET.get("date")
    selected_role = get_role or None
    selected_date = get_date or None
    ...
    # Build role filter queryset for SHARED mode
    if selected_role:
        try:
            selected_role_id = int(selected_role)
    ...
    if selected_role_id:
        filtered_roles_qs = Role.objects.filter(id=selected_role_id)
    else:
        filtered_roles_qs = Role.objects.filter(opportunity=opp_id).select_related("opportunity__organisation")

    # Prefetch Sections for the filtered roles to avoid per‑role queries
    filtered_roles_qs = filtered_roles_qs.prefetch_related(
        Prefetch("section_set", queryset=Section.objects.all(), to_attr="prefetched_sections")
    )
    filtered_roles = list(filtered_roles_qs)

    # Fast path if rota is PERROLE (aka non‑shared/discrete logic kept as helper)
    if getattr(opportunity, "rotaconfig", "SHARED") != "SHARED":
        try:
            ...
            # Keep parity with original debug logging
            print(f"[debug] Error: {e}")
            shifts = []
    else:
        # SHARED schedule logic:
        # 1) Build schedules (OneOffDate) with minimal queries based on filters
        # 2) Cross product with applicable roles
        # 3) Attach sections and compute volunteer lists

        schedules_qs = OneOffDate.objects.filter(
            opportunity=opportunity,
            role__isnull=True,  # SHARED means dates not pinned to a role
            date__gte=datetime.now().date(),
        ).select_related("opportunity__organisation")

        # If a specific date slot was picked, find its (date, start, end) and re‑use across all schedules
        if selected_date:
            try:
                # Guard access to only this opportunity and role‑less in SHARED
                selected_slot = OneOffDate.objects.only("date", "start_time", "end_time").get(
                    id=selected_date,
                    opportunity=opportunity,
                )
                schedules_qs = OneOffDate.objects.filter(
            ...
            except OneOffDate.DoesNotExist:
                schedules_qs = schedules_qs.none()

        schedules = list(schedules_qs)

        # Pre-materialize for fast iteration
        shifts: List[Dict[str, Any]] = []

        # Build shift skeletons, deferring expensive volunteer calls to the last possible moment
        for schedule in schedules:
            for role in filtered_roles:
                # Sections are already prefetched in role.prefetched_sections (empty list if none)
                role_sections = getattr(role, "prefetched_sections", [])
                ...
                if role_sections:
                    for section in role_sections:
                        shifts.append({
                            ...
                            "role": role,
                            "schedule": schedule,
                            # Defer these calls until after list is assembled (still same shape)
                            "available_volunteers": None,
                            "assigned_volunteers": None,
                            "accepted_volunteers": None,
                        })
                else:
                    shifts.append({
                        "section": None,
        ...
        # Populate volunteer-related lists in a tight loop to keep code readable yet efficient
        # Note: These helper functions encapsulate their own optimizations/caching if available.
        for shift in shifts:
            schedule_id = shift["schedule"].id
            role_id = shift["role"].id
            section_id = shift["section"].id if shift["section"] else None

            shift["available_volunteers"] = get_shift_volunteers(schedule_id, role_id)
            if section_id:
                shift["assigned_volunteers"] = get_assigned_volunteers(
                    schedule_id, role_id, section_id, omit_unconfirmed=True
                )
                shift["accepted_volunteers"] = get_accepted_and_confirmed(
                    schedule_id, role_id, section_id
                )
            else:
                shift["assigned_volunteers"] = get_assigned_volunteers(
                    schedule_id, role_id, omit_unconfirmed=True
                )
                shift["accepted_volunteers"] = get_accepted_and_confirmed(
                    schedule_id, role_id
                )

    # Unconfirmed shifts: one query + python filter for active registrations (matches original)
    unconfirmed_qs = VolunteerShift.objects.filter(
        registration__opportunity=opportunity,
        confirmed=False
    ).select_related("registration")

    try:
        # Maintain behavior: keep only shifts with active registrations
        unconfirmed_shifts = [s for s in unconfirmed_qs if s.registration.get_registration_status() == "active"]
    except Exception as e:
        print(f"[debug] Error: {e}")
        unconfirmed_shifts = list(unconfirmed_qs)

    # Sort shifts in ascending order by number of available volunteers
    try:
        if isinstance(shifts, list):
            shifts.sort(key=lambda d: len(d.get("available_volunteers") or []))
    except Exception as e:
        print(f"[debug] Error: {e}")

    # Time slots (date+start+end) aggregation remains value-based for speed and determinism
    slots = (
        OneOffDate.objects
