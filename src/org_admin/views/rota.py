...
from rota.models import Role, Section, VolunteerShift, OneOffDate, VolunteerOneOffDateAvailability, \
    VolunteerRoleIntrest, Occurrence
...
def assign_rota(request, opp_id):
...
def edit_rota_schedule(request, opp_id=None, schedule_id=None, role=None):
    opportunity = None
    schedule = None

    if request.method == "POST":
        if not schedule_id:
            return create_schedule(request, opp_id)
        else:
...
def create_schedule(request, opp_id):
    opportunity = Opportunity.objects.get(id=opp_id)
...
def opportunity_rota_index(request, opportunity_id, error=None, success=None):
    ...
    schedules = OneOffDate.objects.filter(opportunity=opportunity)
